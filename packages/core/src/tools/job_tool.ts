...
export class JobCreateTool extends BaseTool<JobCreateParams, ToolResult> {
  static readonly Name = 'job_create';

  constructor() {
    super(
      JobCreateTool.Name,
      'Create Job',
      'Creates a new job and populates it with an initial set of tasks.',
      {
        type: 'object',
        properties: {
          description: {
          ...
          tasks: {
            ...
            items: {
              ...
              required: ['content', 'priority', 'tasks'],
            },
          },
        },
...
export class JobUpdateTool extends BaseTool<JobUpdateParams, ToolResult> {
  static readonly Name = 'job_update';

  constructor() {
    super(
      JobUpdateTool.Name,
      'Update Job',
      'Updates the properties of a specific Job and adds a comment describing details of the tasks result.',
      {
        type: 'object',
        properties: {
          job_id: {
...
export class JobGetStatusTool extends BaseTool<JobGetStatusParams, ToolResult> {
  static readonly Name = 'Job_get_status';
    ...
    try {
      ...
      const status = {
        ...
        tasks: {
          ...
          in_progress: job.tasks.filter((t) => t.status === 'in_progress')
            .length,
...
export class TaskCreateTool extends BaseTool<TaskCreateParams, ToolResult> {
  static readonly Name = 'task_reate';
...
export class JobGetTasksTool extends BaseTool<JobGetTasksParams, ToolResult> {
  static readonly Name = 'job_get_tasks';
  ...
  constructor() {
    super(
      ...
      'Modifies one or more properties of an existing task and optionally adds a comment.',
      {
        ...
        required: ['task_id', 'content', 'status', 'priority', 'comment'],
      },
      true, // isOutputMarkdown
      false, // canUpdateOutput
      true, // hideFromUI
...
export class JobListTool extends BaseTool<JobListParams, ToolResult> {
  ...
  async execute(params: JobListParams): Promise<ToolResult> {
    try {
      ...
      const displayText = jobSummaries
        .map(
          (job) =>
            `Job ID: ${job.id}\n` +
            `Description: ${job.description}\n` +
