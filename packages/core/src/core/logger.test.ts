...
// Derive the test-specific directory in the same way the logger does.
const projectDir = process.cwd();
const hash = crypto.createHash('sha256').update(projectDir).digest('hex');
const TEST_GEMINI_DIR = path.join(os.homedir(), GEMINI_DIR_NAME, hash);
...
async function cleanupLogAndCheckpointFiles() {
  try {
    // Attempt to remove the entire hashed directory for this project.
    // This is cleaner and more robust than trying to delete individual files.
...
describe('Logger', () => {
  ...
  describe('initialize', () => {
    it('should create .gemini directory and an empty log file if none exist', async () => {
      // The beforeEach hook already creates the logger and initializes it,
      // which in turn creates the directory and file.
      // We just need to verify the outcome.
  ...
  describe('loadCheckpoint', () => {
    ...
    beforeEach(async () => {
      // This beforeEach in the inner describe block will run *after* the outer one.
      // The directory is already created, so we can safely write the file.
      await fs.writeFile(
