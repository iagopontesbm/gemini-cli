...
import { vi, describe, it, expect, beforeEach, afterEach, Mock, beforeAll } from 'vitest';
...
describe('loadGeminiIgnorePatterns', () => {
  ...
  it('should load and parse patterns from .geminiignore, ignoring comments and empty lines', () => {
    ...
    mockedFsReadFileSync.mockImplementation((p: string, encoding: string) => {
      ...
      throw new Error(`Mock fs.readFileSync: Unexpected call with path: ${p}, encoding: ${encoding}`);
    });
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Loaded 5 patterns from .geminiignore'));
  ...
  it('should return an empty array if .geminiignore is empty', () => {
    ...
    const patterns = loadGeminiIgnorePatterns(tempDir);
    expect(patterns).toEqual([]);
    expect(consoleLogSpy).not.toHaveBeenCalledWith(expect.stringContaining('Loaded 0 patterns from .geminiignore'));
    expect(consoleLogSpy).not.toHaveBeenCalledWith(expect.stringContaining('No .geminiignore file found'));
    expect(mockedFsReadFileSync).toHaveBeenCalledWith(ignoreFilePath, 'utf-8');
  });
  ...
  it('should handle read errors (other than ENOENT) and log a warning', () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      expect.stringContaining(`[WARN] Could not read .geminiignore file at ${ignoreFilePath}: Test read error`)
    );
    expect(mockedFsReadFileSync).toHaveBeenCalledWith(ignoreFilePath, 'utf-8');
  });
  ...
  it('should correctly handle patterns with inline comments if not starting with #', () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Loaded 1 patterns from .geminiignore'));
    expect(mockedFsReadFileSync).toHaveBeenCalledWith(ignoreFilePath, 'utf-8');
  });
}); 