# dolphin-cli: Terms of Service and Privacy Notice

dolphin-cli is an open-source tool that allows you to interact with Google's powerful language models directly from your command-line interface. The terms of service and privacy notices that apply to your usage of dolphin-cli depend on the type of account you use to authenticate with Google.
...
- Terms of Service: Your use of dolphin-cli is governed by the general [Google Terms of Service](https://policies.google.com/terms?hl=en-US).
...
## 2. API Key (Using Google Gemini Developer [API](https://ai.google.dev/gemini-api/docs) a: Unpaid Service, b: Paid Service)

If you are using a Google Gemini API key (set via `DOLPHIN_CLI_API_KEY`) for authentication, the following terms apply:
...
If you are using an API key with a Vertex AI Gen API backend (set via `GOOGLE_API_KEY`):
...
## Frequently Asked Questions (FAQ) for dolphin-cli
...
- **Auth method 2a:** Yes, When you use the Google Gemini API key (Unpaid Service) terms apply. Under this notice , your **prompts, answers, and related code are collected** and may be used to improve Google's products, which includes model training.
- **Auth method 2b, 3 & 4:** No. For these accounts, your data is governed by the Google Cloud or Google Gemini API (Paid Service) terms, which treat your inputs as confidential. Your code, prompts, and other inputs are **not** used to train models.

### 2. What are "Usage Statistics" and what does the opt-out control?

The "Usage Statistics" setting is the single control for all optional data collection in the dolphin-cli. The data it collects depends on your account type:
