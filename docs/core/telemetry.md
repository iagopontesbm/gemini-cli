...
You can enable telemetry in two primary modes. All configuration happens in your `.gemini/settings.json` file and via environment variables.

First, enable the master switch in `.gemini/settings.json`:
```json
{
  "telemetry": true
}
...
* `gemini_cli.tool.call.duration` (Histogram, ms): Measures tool call latency.
    * **Attributes**:
        * `function_name`
...
* `gemini_cli.api.request.duration` (Histogram, ms): Measures API request latency.
    * **Attributes**:
        * `model`

* `gemini_cli.api.token.prompt_count` (Histogram, Int): Measures the number of tokens in prompts sent to the API.
    * **Attributes**:
        * `model`
