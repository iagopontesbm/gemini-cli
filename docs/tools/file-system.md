# dolphin-cli file system tools

The dolphin-cli provides a comprehensive suite of tools for interacting with the local file system. These tools allow the Google Gemini model to read from, write to, list, search, and modify files and directories, all under your control and typically with confirmation for sensitive operations.
...
- **Behavior:**
  ...
  - If the initial `old_string` isn't found or matches multiple locations, the tool can leverage the Google Gemini model to iteratively refine `old_string` (and potentially `new_string`).
...
These file system tools provide a foundation for the dolphin-cli to understand and interact with your local project context.
