# MCP servers with the dolphin-cli

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with the dolphin-cli.

## What is an MCP server?

An MCP server is an application that exposes tools and resources to the dolphin-cli through the Model Context Protocol, allowing it to interact with external systems and data sources. MCP servers act as a bridge between the Google Gemini model and your local environment or other services like APIs.

An MCP server enables the dolphin-cli to:
...
- **Access resources:** Read data from specific resources (though the dolphin-cli primarily focuses on tool execution).

With an MCP server, you can extend the dolphin-cli's capabilities to perform actions beyond its built-in features, such as interacting with databases, APIs, custom scripts, or specialized workflows.

## Core Integration Architecture

The dolphin-cli integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
4. **Sanitizes and validates** tool schemas for compatibility with the Google Gemini API
...
The dolphin-cli supports three MCP transport types:
...
The dolphin-cli uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.

### Configure the MCP server in settings.json

You can configure MCP servers at the global level in the `~/.dolphin-cli/settings.json` file or in your project's root directory, create or open the `.dolphin-cli/settings.json` file. Within the file, add the `mcpServers` configuration block.
...
When the dolphin-cli starts, it performs MCP server discovery through the following detailed process:
...
3. **Name sanitization:** Tool names are cleaned to meet Google Gemini API requirements:
...
Tool parameter schemas undergo sanitization for Google Gemini API compatibility:
...
When the Google Gemini model decides to use an MCP tool, the following execution flow occurs:
...
Once discovered, MCP tools are available to the Google Gemini model like built-in tools. The model will automatically:
...
- **Property stripping:** The system automatically removes certain schema properties (`$schema`, `additionalProperties`) for Google Gemini API compatibility
- **Name sanitization:** Tool names are automatically sanitized to meet API requirements
- **Conflict resolution:** Tool name conflicts between servers are resolved through automatic prefixing

This comprehensive integration makes MCP servers a powerful way to extend the dolphin-cli's capabilities while maintaining security, reliability, and ease of use.
