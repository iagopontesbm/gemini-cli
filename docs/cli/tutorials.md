...
This page contains tutorials for interacting with dolphin-cli.
...
In your project's root directory, create or open the [`.dolphin-cli/settings.json` file](./configuration.md). Within the file, add the `mcpServers` configuration block, which provides instructions for how to launch the GitHub MCP server.
...
dolphin-cli uses this value in the `mcpServers` configuration that you defined in the `settings.json` file.

#### Launch dolphin-cli and verify the connection

When you launch dolphin-cli, it automatically reads your configuration and launches the GitHub MCP server in the background. You can then use natural language prompts to ask dolphin-cli to perform GitHub actions. For example:
