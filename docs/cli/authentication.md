...
The dolphin-cli requires you to authenticate with Google's AI services. On initial startup you'll need to configure **one** of the following authentication methods:

1.  **Login with Google (Gemini Code Assist):**

    - Use this option to log in with your google account.
    - During initial startup, dolphin-cli will direct you to a webpage for authentication. Once authenticated, your credentials will be cached locally so the web login can be skipped on subsequent runs.
    - Note that the web login must be done in a browser that can communicate with the machine dolphin-cli is being run from. (Specifically, the browser will be redirected to a localhost url that dolphin-cli will be listening on).
...
2.  **<a id="dolphin-cli-api-key"></a>dolphin-cli API key (for Google Gemini API):**

    - Obtain your API key from Google AI Studio: [https://aistudio.google.com/app/apikey](https://aistudio.google.com/app/apikey)
    - Set the `DOLPHIN_CLI_API_KEY` environment variable. In the following methods, replace `YOUR_API_KEY` with the API key you obtained from Google AI Studio:
      ...
      - For repeated use, you can add the environment variable to your `.env` file (located in the project directory or user home directory) or your shell's configuration file (like `~/.bashrc`, `~/.zshrc`, or `~/.profile`). For example, the following command adds the environment variable to a `~/.bashrc` file:
        ```bash
        echo 'export DOLPHIN_CLI_API_KEY="YOUR_API_KEY"' >> ~/.bashrc
        source ~/.bashrc
        ```
